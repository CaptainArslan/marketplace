                                ...
                                <div class="card text-center">
                                    ...
                                    @if (!is_null($alreadybuy))
                                        @if ($plan->id != 1 && $alreadybuy->sub_id != $plan->id)
                                            <a
                                                data-value={{ $plan }}data-ubalance={{ getAmount(auth()->user()->balance) }}class="buyBtn">@lang('Buy Now')</a>
                                        @elseif($alreadybuy->sub_id == $plan->id)
                                            {{-- <a data-value={{ $plan }}data-ubalance={{ getAmount(auth()->user()->balance) }}>@lang('Current Plan')</a> --}}
                                            <a
                                                data-value={{ $plan }}data-ubalance={{ getAmount(auth()->user()->balance) }}class="cancelBtn">@lang('Cancel Plan')</a>
                                        @else
                                        @endif
                                    @endif
                                    @if (is_null($alreadybuy))
                                        @if ($plan->id == 1)
                                            <a data-value={{ $plan }}
                                                data-ubalance={{ getAmount(auth()->user()->balance) }}>@lang('Current Plan')</a>
                                        @else
                                            <a data-value={{ $plan }}
                                                data-ubalance={{ getAmount(auth()->user()->balance) }}
                                                class="buyBtn">@lang('Buy Now')</a>
                                        @endif
                                    @endif
