                    ...
                    <div class="card-header">
                        <div class="row">
                            <div class="col-sm-10 d-flex flex-wrap align-items-center">
                                ...
                                @endif
                                <h4 class="ms-2">[@lang('Ticket') :#{{ $my_ticket->ticket }}]
                                    {{ $my_ticket->subject }}
                                </h4>
                            </div>
                            <div class="col-sm-2 text-end">
                                ...
                                @endphp
                                <a href="{{ $url }}"><button class="btn btn--primary btn-sm" data-bs-toggle="modal" data-bs-target="#DelModal"><i class="las la-arrow-left"></i></button></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        @php
                        $url = $partial ? route('ticket.reply', $my_ticket->id) : route('iframe.api.ticket.reply', $my_ticket->id)."?token=" . request()->token;
                        @endphp
                        ...
                        <form method="post" action="{{ $url }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group text-end">
                                <button type="submit" class="btn-lg btn--base w-100 py-2" name="replayTicket" value="1">@lang('Reply')</button>
                            </div>
                        </form>
                        @endif

                        @foreach ($messages as $message)
                        @if ($message->admin_id == 0 && $message->seller_id == 0)
                        <div class="single-reply">
                            <div class="left flex">
                                <h4>{{ $message->ticket->name }}</h4>
                            </div>
                            <div class="right">
                                <span class="fst-italic font-size--14px text--base mb-2">@lang('Posted on')
                                    {{ $message->created_at->format('l, dS F Y @ H:i') }}</span>
                                <p>{{ $message->message }}</p>

                                @if ($message->attachments()->count() > 0)
                                <div class="attachment-list mt-2">
                                    @foreach ($message->attachments as $k => $image)
                                    <a href="{{ route('ticket.download', encrypt($image->id)) }}" class="mr-3"><i class="fa fa-file"></i> @lang('Attachment')
                                        {{ ++$k }}</a>
                                    @endforeach
                                </div>
                                @endif
                        ...
                        @elseif($message->admin_id != 0 && $message->seller_id == 0)
                        <div class="single-reply admin-reply">
                            ...
                            <div class="single-reply admin-reply">
                                <div class="left">
                                    <h4>@lang('Seller') {{$seller->username ?? ''}}</h4>
                                </div>
