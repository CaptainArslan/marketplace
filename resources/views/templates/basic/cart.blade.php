            ...
            <div class="row gy-4 justify-content-center">
                                ...
                                <div class="single-cart">
                                    ...
                                    <div class="single-cart__content">
                                        ...
                                        <span class="fs-14px">@lang('Product by') - <a
                                                ...
                                                class="text-decoration-underline text--base">{{ $item->author->username }}</a></span>
                                    ...
                                    <div class="single-cart__price">
                                        ...
                                        <div class="price">{{ $general->cur_sym }}{{ getAmount($item->total_price) }}
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal fade" id="cartRemoveModal{{ $loop->index }}"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <form action="{{ route('remove.cart', Crypt::encrypt($item->id)) }}"
                                                    method="GET">
                                                    @csrf
                ...
                <div class="col-lg-4 ps-xl-5 ps-lg-4">
                    <div class="cart-total-box text-center">
                        ...
                        <div class="totoal-price text-white">{{ $general->cur_sym }}{{ $orders->sum('total_price') }}
                        </div>
                        @if ($ordersCount > 0)
                            @auth
                                <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#paymentModal"
                                    class="btn btn--light bg-white w-100 mt-4">@lang('Checkout Now')</a>
                            @else
                                <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#loginMessageModal"
                                    class="btn btn--light bg-white w-100 mt-4">@lang('Checkout Now')</a>
                            @endauth
                        @endif
                    </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <select name="wallet_type" class="form--control" required>
                                            <option value="own" @if (getAmount(auth()->user()->balance) < $orders->sum('total_price')) disabled @endif>
                                                @lang('Own Wallet') -
                                                {{ $general->cur_sym }}{{ getAmount(auth()->user()->balance) }}</option>
