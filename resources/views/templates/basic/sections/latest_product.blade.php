@php
    ...
    $latestProducts = \App\Product::where('status', 1)
        ->where('total_sell', '>', 1)
    ...
    $categoyData = \App\Category::where('status', 1)
        ->with([
            'products' => function ($q) {
                $q->where('status', 1)
                    ->where('total_sell', '>', 1)
                ...
                <ul class="nav nav-tabs custom--nav-tabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#all-tab" type="button"
                            role="tab">@lang('All Categoires')</button>
                    </li>

                    @foreach ($categoyData as $item)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" data-bs-toggle="tab"
                                data-bs-target="#{{ str_replace(' ', '_', strtolower($item->name)) }}" type="button"
                                role="tab">{{ __($item->name) }}</button>
                        </li>
                    @endforeach
                            ...
                            <div class="product-card style--three p-0">
                                ...
                                <div class="product-card__content bg-white">
                                    <p class="mb-1">@lang('by') <a
                                            ...
                                            class="text--base">{{ __($item->user->username) }}</a> @lang('in') <a
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="product-card style--three p-0">
                                    ...
                                    <div class="product-card__content bg-white">
                                        ...
                                        <p>@lang('by') <a
                                                href="{{ route('username.search', strtolower($data->user->username)) }}">{{ $data->user->username }}</a>
                                            @lang('in') <a
                                                href="{{ route('subcategory.search', [$data->subcategory->id, str_slug($data->subcategory->name)]) }}">{{ __($data->subcategory->name) }}</a>
                                        </p>

                                        <div class="product-card__meta">
                                            <div class="left">
                                                <h5 class="product-price mb-2">
                                                    {{ __($general->cur_sym) }}{{ getAmount($data->regular_price) }}
                                                </h5>
                                                <ul class="meta-list">
                                                    <li><i class="las la-download"></i> {{ $data->total_sell }}
                                                        @lang('Sales')</li>
                                                </ul>
                                            </div>
                                            <div class="right">
