...
class CategoryController extends Controller
{
    ...
    {
        $categories= OtherCategory::with('othercategoryproduct')->paginate(getPaginate());
        $page_title = 'other Categories';
        $empty_message = 'No data found';
        return view('admin.category.othercategory', compact('page_title', 'categories', 'empty_message'));
    }
    ...
    public function activate(Request $request)
    {
        ...
        $category->status = 1;
        $category->save();
        $othercategory=OtherCategory::Where('category_name',"like",$category->name)->first();
        if(!empty($othercategory)){
        if(!empty($othercategory->subcategory_name)){
            $subcategory = SubCategory::where('category_id','=',$category->id)->first();
            $subcategory->status = 1;
            $subcategory->save();
        }
        $products=OtherCategoryProduct::Where('othercategory_id','=',$othercategory->id)->get();
    ...
    public function searchOtherCategory(Request $request)
    {

        $search = $request->search;
        $page_title = 'Category Search - ' . $search;
        $empty_message = 'No data found';
        $categories = OtherCategory::where('category_name', 'like', "%$search%")->with('othercategoryproducts')->paginate(getPaginate());
    ...
    public function storeSubcategory(Request $request, $id)
    {
        $request->validate([
            ...
            'shortCode' => 'required|string|max:150',
        ]);
        ...
        $subcategory->shortcode = $request->shortCode;
        $subcategory->status = 1;
        $subcategory->save();
